# Images Tab Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Add an **Images** tab to the popup's SEO cluster — a per-page audit of every image (alt text, size, format, dimensions, loading, broken status) in a filterable, sortable table, mirroring the existing Links tab.

**Architecture:** A new content-script handler (`get_images`) extracts image data from the page using the DOM + Resource Timing API (no new network requests). `highlight_images` / `scroll_to_image` provide on-page interaction. The popup's `utils.ts` wraps these in `getImages` / `highlightImages` / `scrollToImage` / `analyzeImages`. A new `Images.svelte` renders the UI, wired into `App.svelte`'s already-stubbed `images` tab slot.

**Tech Stack:** WXT, Svelte 5 (Runes), TypeScript, browser extension messaging.

---

## Testing note

This project has **no unit-test runner** (no `test` script; no vitest/jest). Verification is:

- **Per commit:** the repo's pre-commit hook runs `oxlint` + `oxfmt --check` automatically — every commit is a lint/format gate.
- **Types:** run `bun run typecheck` once at the end (Task 9). During iteration, rely on editor/HMR diagnostics — do not run dev/build/typecheck mid-iteration (the user keeps HMR running).
- **Behavior:** manual dogfood via the `/browse` skill (Task 9).

Because there is no test framework, tasks below are **implement → commit**, not TDD red/green. Each task is a self-contained, committable change.

## File structure

| File                              | Responsibility                   | Change                                                                                        |
| --------------------------------- | -------------------------------- | --------------------------------------------------------------------------------------------- |
| `utils/analytics.ts`              | Typed analytics actions          | Add 7 `images_*` actions to the union + `TIME_SAVINGS` + `ACTION_CATEGORIES`                  |
| `entrypoints/popup/types.ts`      | Shared popup types               | Add `RawImage`, `ImageSource`, `ImageLoading`, `ImageStatus`                                  |
| `entrypoints/main.content.ts`     | In-page extraction & interaction | Add `collectImageEls()` helper + `get_images`, `highlight_images`, `scroll_to_image` handlers |
| `entrypoints/popup/utils.ts`      | Popup ⇄ content-script bridge    | Add `getImages`, `highlightImages`, `scrollToImage`, `analyzeImages`                          |
| `entrypoints/popup/Images.svelte` | Images tab UI                    | New component (built from `Links.svelte`)                                                     |
| `entrypoints/popup/App.svelte`    | Popup shell / tab wiring         | Enable `images` tab: type, icon, `seoTabs` entry, state, fetch, render                        |

---

### Task 1: Analytics actions

`AnalyticsAction` is a strict union with two exhaustive companion maps (`TIME_SAVINGS`, `ACTION_CATEGORIES`). Adding actions in only one place fails typecheck. Add all three together, mirroring the `links_*` entries.

**Files:**

- Modify: `utils/analytics.ts`

- [ ] **Step 1: Add the 7 image actions to the union**

In `utils/analytics.ts`, find the `assets_sort` line that ends the union (`  | 'assets_sort';`) and replace it so the union continues with the image actions:
...
In the `TIME_SAVINGS` map, find `  assets_sort: 60` (the last entry, followed by `};`) and replace with:
...
In the `ACTION_CATEGORIES` map, find `  assets_sort: 'SEO'` (the last entry, followed by `};`) and replace with:
...
git commit -m "feat(analytics): add images tab actions"
```

The pre-commit hook runs lint + format; a clean commit means both passed.
...
- [ ] **Step 1: Append the image types**

Add to the end of `entrypoints/popup/types.ts` (after the last interface, before EOF):
...
git commit -m "feat(images): add RawImage types"
```

---

### Task 3: Content-script extraction (`get_images`)

The extraction order must be identical to the highlight/scroll handlers (Task 4) so indices line up. A single shared `collectImageEls()` guarantees that.

**Files:**

- Modify: `entrypoints/main.content.ts`

- [ ] **Step 1: Add the shared collector at module scope**

In `entrypoints/main.content.ts`, after the imports (after line 3, the `handleReturnUrlRedirect` import) and before `export default defineContentScript({`, add:
...
- [ ] **Step 2: Add the `get_images` handler**

In `entrypoints/main.content.ts`, locate the end of the `get_assets` handler — the lines:
...
if (request.action === 'get_images') {
  ...
  const cachedOf = (t?: PerformanceResourceTiming): boolean => !!t && t.transferSize === 0 && t.decodedBodySize > 0;
...
git commit -m "feat(images): extract page images in content script"
```

---

### Task 4: Content-script highlight + scroll

**Files:**

- Modify: `entrypoints/main.content.ts`

- [ ] **Step 1: Add `highlight_images` and `scroll_to_image` handlers**

In `entrypoints/main.content.ts`, locate the end of the `scroll_to_heading` handler — the block that ends with:
...
Insert the following **between** that handler's closing `}` and the `// Return false for unhandled messages` comment:
...
```bash
git add entrypoints/main.content.ts
git commit -m "feat(images): add image highlight and scroll-to handlers"
```

---

### Task 5: Popup bridge (`utils.ts`)

**Files:**

- Modify: `entrypoints/popup/utils.ts`

- [ ] **Step 1: Import `RawImage`**

In `entrypoints/popup/utils.ts`, change the first import line:
...
- [ ] **Step 2: Add the image helpers**

Append to the end of `entrypoints/popup/utils.ts`:
...
git commit -m "feat(images): add popup image bridge helpers"
```

---

### Task 6: `Images.svelte` component

Built from `Links.svelte`, adapted to four filter facets (Alt / Format / Loading / Status), a thumbnail-led table, and image-specific export. Reuses the same CSS class names/tokens as Links.

**Files:**

- Create: `entrypoints/popup/Images.svelte`

- [ ] **Step 1: Create the component**

Create `entrypoints/popup/Images.svelte` with exactly:
...
```bash
git add entrypoints/popup/Images.svelte
git commit -m "feat(images): add Images tab component"
```

---

### Task 7: Wire the tab into `App.svelte`
...
Add an import after the `Assets` import (line 7):

```ts
import Images from './Images.svelte';
```

Change line 10 to add `RawImage`:

```ts
import type { StoreInfo, RawHeading, RawLink, RawAsset, RawImage } from './types';
```

- [ ] **Step 2: Update the `TabId` type and the Future-tabs comment**
...
- [ ] **Step 3: Add state + derived badge count**

After the `let rawAssets = $state<RawAsset[]>([]);` line (line 34), add:

```ts
let rawImages = $state<RawImage[]>([]);
```

After the `const headingIssues = $derived(analyzeHeadings(rawHeadings));` line (line 37), add:

```ts
const imageIssueCount = $derived(analyzeImages(rawImages));
```

- [ ] **Step 4: Add the `images` entry to `seoTabs`**

In the `seoTabs` `$derived` array, replace the line:
...
(Also delete the now-redundant `// { id: 'images', ... }` comment line in that array if present.)

- [ ] **Step 5: Enable the `images` icon**

In the `tabIcon` snippet, find the `assets` branch:
...
Insert a new branch immediately after it (before the `<!-- Future tab icons` comment):
...
Then remove the now-duplicate `images` line from inside the commented `Future tab icons` block.

- [ ] **Step 6: Fetch images on load**

In the `fetchData` function, change:

```ts
const [storeData, headingsData, linksData, assetsData] = await Promise.all([
  getTheme(),
  getHeadings(),
  getLinks(),
  getAssets()
...
And after `rawAssets = assetsData;` add:
...
In the content render block, find:
...
git commit -m "feat(images): wire Images tab into popup"
```
...
- Modify: `changelog.json` (via the `/version-bump` skill)

- [ ] **Step 1: Add the changelog + version bump**

Invoke the `/version-bump` skill (project convention — it handles CalVer + `changelog.json`). Use a feature entry titled **"SEO Images Analyzer"** describing: per-page image audit; alt-text, size, format, dimensions, loading, broken-status columns; Alt/Format/Loading/Status filters + search; highlight-on-page; row-click scroll; CSV/JSON/copy export. Do not hand-edit `changelog.md` (it is generated).

- [ ] **Step 2: Commit** (the skill stages its own changes; commit if it does not)

```bash
git add changelog.json package.json wxt.config.ts
git commit -m "chore: bump version for images tab"
```
...
Expected: no errors. (Common failure: a missing `images_*` analytics entry from Task 1 → add it.)

- [ ] **Step 2: Lint + format**

Run: `bun run lint && bun run format:check`
Expected: no errors (warnings about pre-existing `no-underscore-dangle` in other files are fine).

- [ ] **Step 3: Manual dogfood**

Use the `/browse` skill (or load the unpacked build) and open the popup on:

1. A Shopify storefront on Dawn — verify thumbnails, sizes (Shopify CDN exposes timing), formats, dims, OK status.
2. A page with missing-alt images — verify red "Missing alt text", red `Alt` pill, sidebar badge count, and that `analyzeImages` count matches the Empty filter count.
3. A page with `<picture>`/srcset — verify the resolved `currentSrc` URL/format is shown and `isResponsive` images appear once (not duplicated).
4. A page with CSS background heroes — verify `bg` tag, `—` in Alt/Load, and they still get a thumbnail + size where timing exists.
5. A non-Shopify page — the Images tab must still work (SEO tabs are not Shopify-gated).

For each: exercise all four filters, search, every sortable column (Size/Dims/Status), the Highlight toggle (outlines appear/clear), a row click (scrolls + highlights on page), and all three exports (CSV/JSON/Copy).

- [ ] **Step 4: Final commit (only if Steps 1–2 required fixes)**

```bash
git add -A
git commit -m "fix(images): resolve typecheck and lint issues"
```
...
- Scope (img / picture / background) → Task 3 `collectImageEls` + `get_images`. ✓
- Data model `RawImage` → Task 2. ✓
- Alt semantics (missing or empty → flagged, bg exempt) → Task 3 `lacksAlt`, Task 6 `statusOf`. ✓
- Broken best-effort (`naturalWidth === 0`) → Task 3 + Task 4. ✓
- Size via Resource Timing only → Task 3 (no fetch). ✓
- UI: header/filters/search/table/status pills/footer-equivalent count → Task 6. ✓
- Highlight + scroll with consistent index → shared `collectImageEls` (Tasks 3–4). ✓
- Export CSV/JSON/copy → Task 6. ✓
- App wiring incl. badge → Task 7. ✓
- Analytics (strict union) → Task 1. ✓

**Type consistency:** `RawImage`/`ImageSource`/`ImageLoading`/`ImageStatus` defined in Task 2 are imported in Tasks 5–7; `statusOf` returns `ImageStatus`; `analyzeImages` signature matches Task 5 ↔ Task 7 usage; the content-script object shape in Task 3 matches `RawImage` field-for-field. The `images_*` action strings in Task 1 match every `trackAction(...)` call in Task 6. ✓

**Placeholder scan:** No TBD/TODO; all code blocks are complete; no "similar to" references. ✓

**Note on `collectImageEls` ordering:** indices are positions in `[...imgs, ...backgrounds]`. They are consistent across `get_images`/`highlight_images`/`scroll_to_image` because all three call the same function, which is what the scroll/highlight contract requires (it does not need visual DOM order across the two groups).
