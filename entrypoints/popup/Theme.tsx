...
function InfoItem({ label, value, type = 'text', isLast = false }: InfoItemProps) {
  ...
  return (
    <div className={`flex justify-between items-center py-3.5 ${!isLast ? 'border-b border-slate-100' : ''}`}>
      ...
      <span className={`max-w-[60%] break-all text-right select-text ${getValueClassName()}`}>{value}</span>
    </div>
  );
}
...
export default function Theme({ storeInfo }: { storeInfo: StoreInfo }) {
  ...
  return (
    <div className="flex flex-col">
      ...
      <InfoItem label="Theme name:" value={storeInfo.theme?.schema_name || storeInfo.theme?.name || 'N/A'} />
