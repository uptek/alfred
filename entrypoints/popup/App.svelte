<script lang="ts">
  ...
  // Future tabs: 'apps' | 'products' | 'overview' | 'headings' | 'links' | 'hreflangs' | 'images' | 'schema' | 'social' | 'robots' | 'sitemaps'
  ...
  //   { id: 'headings', label: 'Headings', icon: 'headings' },
...
{:else if !storeInfo?.isShopify}
  <div class="popup popup--compact">
    <div class="empty">
      ...
      <p>Navigate to a Shopify store to see Alfred's magic ✨</p>
    </div>
  </div>
{:else}
  <div class="popup">
    ...
    <div class="layout">
      ...
      <nav class="sidebar" aria-label="Alfred navigation">
        <!-- Shopify section label (uncomment when multiple Shopify tabs exist)
        <div class="sidebar__label">
          <svg width="70" height="20" viewBox="0 0 100 28.6" aria-hidden="true" class="sidebar__shopify-logo">
        ...
        <!-- SEO section (uncomment when SEO tabs are implemented)
...
<style>
  ...
  :global(.sidebar__shopify-logo) { opacity: 0.55; }
  ...
  /* Empty block */
  .empty { display: flex; flex-direction: column; align-items: center; justify-content: center; height: 100%; gap: 8px; padding: 40px; text-align: center; }
  .empty h3 { font-size: 16px; font-weight: 600; color: var(--btn-bg-hover); margin: 0; }
  .empty p { font-size: 14px; color: var(--text-link-muted); margin: 0; }
</style>
