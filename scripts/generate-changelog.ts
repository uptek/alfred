...
const generateChangelog = async () => {
  try {
    ...
    const changelogHtmlPath = path.resolve(process.cwd(), 'entrypoints/options/sections/changelog.html');

    const changelogData: ChangelogEntry[] = JSON.parse(await fs.readFile(changelogJsonPath, 'utf-8'));

    // Generate changelog.md
    let mdContent = '# Changelog\n\n';
    let firstVideoRendered = false;
    ...
    firstVideoRendered = false; // Reset for HTML generation
    let htmlContent = '<s-stack gap="base">\n';
    htmlContent += '  <s-stack direction="inline" alignItems="center" gap="small-200">\n';
    ...
    changelogData.forEach((entry, index) => {
      ...
      htmlContent += '          <s-stack direction="inline" alignItems="center" gap="small-200">\n';
      htmlContent += `            <s-heading color="strong">${formatDate(entry.date)}</s-heading>\n`;
      if (index === 0) {
